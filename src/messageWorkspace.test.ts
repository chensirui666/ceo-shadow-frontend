...
test('MessageWorkspace content has a readable empty state and switches to the selected detail', async () => {
  const [{ MessageWorkspaceContent }, { createDemoMessageSnapshot }] = await Promise.all([
    vite.ssrLoadModule('/src/components/MessageWorkspace.tsx'), vite.ssrLoadModule('/src/messageState.ts'),
  ])
  const handlers = { onBack: () => {}, onConfirm: () => {}, onFeedback: () => {}, onOpen: () => {}, onSkip: () => {}, onStatusChange: () => {} }
  const empty = renderToStaticMarkup(createElement(MessageWorkspaceContent, { ...handlers, selectedId: null, snapshot: { messages: [] }, status: 'all' }))
  const message = createDemoMessageSnapshot(new Date('2026-08-13T12:00:00.000Z')).messages[0]
  const detail = renderToStaticMarkup(createElement(MessageWorkspaceContent, { ...handlers, selectedId: message.id, snapshot: { messages: [message] }, status: 'all' }))

  assert.match(empty, /还没有需要处理的消息/)
  assert.match(detail, new RegExp(message.question))
  assert.match(detail, /返回消息/)
})
