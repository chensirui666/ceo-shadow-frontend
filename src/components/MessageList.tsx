    ...
    <div className="message-list-layout">
      <section aria-label={copy.list.label} className="message-list-stream">
        ...
        {visibleMessages.length ? visibleMessages.map((message) => <article className="message-list-row" key={message.id}>
          ...
          <span className="message-list-actions">{message.status === 'needs-confirmation' && <><Button className="message-list-action-confirm" onPress={() => onConfirm(message.id)} size="sm" type="button" variant="primary"><Check aria-hidden="true" />{copy.list.confirm}</Button><Button className="message-list-action-skip" onPress={() => onSkip(message.id)} size="sm" type="button" variant="secondary"><SkipForward aria-hidden="true" />{copy.list.skip}</Button></>}{canProvideMessageFeedback(message) && <MessageFeedbackControls copy={copy.feedbackControls} id={message.id} onFeedback={onFeedback} stopPropagation />}</span>
