...
export function ContactsWorkspaceContent({ contacts, selectedId, query, time, sourceIds, connectedSources, copy, onOpen, onAction, onQueryChange, onSourceIdsChange, onTimeChange }: ContactsWorkspaceContentProps) {
  ...
  return <section className={`contacts-page${selected ? ' contacts-page-preview' : ''}`}>
    <div className="contacts-main">
      <header className="contacts-header"><div><h1>{copy.title}</h1><p>{copy.explainer}</p></div><Button className="contacts-get" onPress={() => onAction('', 'refresh')}>{connectedSources.length ? <><BotMessageSquare aria-hidden="true" />{copy.get}</> : copy.goConnect}</Button></header>
...
export default function ContactsWorkspace({ copy, locale, service = createContactsService(), onOpenSettings }: { copy: ContactsCopy; locale: Locale; service?: ContactsService; onOpenSettings: () => void }) {
  ...
  const selected = contacts?.find((contact) => contact.id === selectedId) ?? null
  const update = (action: Promise<Contact[]>, nextNotice: string, onSuccess?: () => void) => { void action.then((next) => { setContacts(next); setNotice(nextNotice); onSuccess?.() }).catch(() => setEditError(copy.editDialog.error)) }
  const openDiscovery = () => {
  ...
  const saveEdit = () => {
    if (!editing) return
    update(service.update(editing.id, { name: editing.name, relationship: editing.relationship, sources: editing.sources, contextPrompt: editing.contextPrompt }), copy.notices.saved, () => setEditing(null))
  }
