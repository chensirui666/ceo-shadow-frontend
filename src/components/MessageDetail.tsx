...
export default function MessageDetail({ copy, message, onBack, onConfirm, onFeedback, onSkip }: MessageDetailProps) {
  ...
  return <section aria-labelledby="message-detail-title" className="message-detail">
    ...
    <div className="message-detail-layout"><main className="message-detail-reading">
      ...
      <section className="message-detail-card message-detail-tasks"><h2><ListChecks aria-hidden="true" />D. {copy.detail.tasks}</h2>{relatedTasks.length > 0 ? <ul>{relatedTasks.map((task) => <li key={task.title}><span>{task.title}</span><strong className={`message-detail-task-status message-detail-task-status-${task.status}`}>{copy.detail.taskStatus[task.status]}</strong></li>)}</ul> : <p>{message.taskSummary}</p>}</section>
      <section className="message-detail-card message-detail-feedback"><div className="message-detail-feedback-header"><h2><MessageSquare aria-hidden="true" />E. {copy.detail.feedback}</h2><MessageFeedbackControls copy={copy.feedbackControls} id={message.id} onFeedback={onFeedback} /></div></section>
