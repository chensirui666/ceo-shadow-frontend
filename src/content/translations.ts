...
type OnboardingCopy = {
  ...
  memory: { title: string; subtitle: string; scope: Record<HomeSource, string>; demo: string; confirm: string; view: string; skip: string; proceed: string; readingTitle: string; reading: [string, string]; summaryTitle: string; summary: string; construct: string; building: string }
  style: { title: string; subtitle: string; summary: string; points: string[]; showPrompt: string; extract: string; extractingTitle: string; extracting: [string, string]; editTitle: string; editHint: string; promptLabel: string; usePrompt: string; demo: string; prompt: string; confirm: string }
  ...
  en: {
    ...
    workspace: {
      ...
      onboarding: {
        ...
        memory: { title: 'Build your Memory', subtitle: 'Review what Friday would read before you create the first work Memory.', scope: { dingtalk: 'Work messages and related calendar context', feishu: 'Work messages and shared document context', teams: 'Messages and channel collaboration context' }, demo: 'Local demo only: no connected content is read or written to Memory.', confirm: 'Build Memory', view: 'View Memory', skip: 'Skip', proceed: 'Confirm', readingTitle: 'Reading connected work context', reading: ['Reading recent work messages…', 'Organising documents and meeting context…'], summaryTitle: 'What Friday found', summary: '126 work messages, 18 documents, and 7 recurring work topics are ready for the first Memory.', construct: 'Build Memory now', building: 'Building your local Memory preview…' },
        style: { title: 'Confirm your work style', subtitle: 'Start with a readable summary. The original Prompt is available when you need it.', summary: 'Friday distilled these four work habits from your work context.', points: ['Expression: concise and explicit about uncertainty.', 'Judgment: check facts, risks, and owner first.', 'Next step: make the next action clear.', 'Boundaries: hand off commitments and sensitive or missing context.'], showPrompt: 'View original Prompt', extract: 'Extract work style', extractingTitle: 'Extracting your work style', extracting: ['Reviewing the connected work context…', 'Preparing the first work-style Prompt…'], editTitle: 'Edit your work-style Prompt', editHint: 'Refine this preview before you confirm it.', promptLabel: 'Work-style Prompt', usePrompt: 'Use this Prompt', demo: 'Local demo only: this preview is not stored or sent anywhere.', prompt: 'Expression\nUse concise language. State uncertainty directly.\n\nDecision order\nConfirm current facts, risks, owners, and due dates before committing.\n\nScenario rules\nFor project and meeting updates, give the next step and owner.\n\nBoundaries\nHand off commitments, sensitive content, and missing context.', confirm: 'This works for me' },
  ...
  zh: {
    ...
    workspace: {
      ...
      onboarding: {
        ...
        memory: { title: '建立你的 Memory', subtitle: '先确认 Friday 将读取的范围，再建立第一份工作 Memory。', scope: { dingtalk: '工作消息与相关日程上下文', feishu: '工作消息与共享文档上下文', teams: '消息与频道协作上下文' }, demo: '仅为本地演示：不会读取已连接应用的内容，也不会写入真实 Memory。', confirm: '建立 Memory', view: '查看 Memory', skip: 'Skip', proceed: '确认', readingTitle: '正在读取已连接的工作信息', reading: ['正在读取近期工作消息…', '正在整理文档与会议上下文…'], summaryTitle: 'Friday 读取到的内容', summary: '已整理 126 条工作消息、18 份文档，以及 7 个反复出现的工作主题，可用于构造第一份 Memory。', construct: '开始构造 Memory', building: '正在构造本地 Memory 预览…' },
        style: { title: '确认你的工作风格', subtitle: '先看易读摘要；需要时再查看 Prompt 原文。', summary: 'Friday 从你的工作材料里提炼出四类习惯。', points: ['表达：简洁直接；不确定会说清楚。', '判断：先确认事实、风险与负责人。', '下一步：更新后说清下一步。', '边界：承诺、敏感事项或信息不足，交给你。'], showPrompt: '查看 Prompt 原文', extract: '提取工作风格', extractingTitle: '正在提取你的工作风格', extracting: ['正在梳理已连接的工作上下文…', '正在生成初版工作风格 Prompt…'], editTitle: '编辑你的工作风格 Prompt', editHint: '确认前可直接修改这份预览。', promptLabel: '工作风格 Prompt', usePrompt: '使用这个 Prompt', demo: '仅为本地演示：此预览不会被持久保存或发送。', prompt: '表达方式\n使用简洁语言，直接说明不确定性。\n\n决策顺序\n先确认当前事实、风险、负责人和截止时间，再做出承诺。\n\n场景规则\n项目和会议更新后，给出下一步与负责人。\n\n必须交由本人\n承诺、敏感内容和信息不足时必须交由本人。', confirm: '符合我，继续' },
