...
type OnboardingCopy = {
  ...
  style: { title: string; subtitle: string; summary: string; points: string[]; benefits: { summary: string; items: [string, string, string] }; extract: string; skip: string; extractingTitle: string; extracting: [string, string, string, string]; editHint: string; promptLabel: string; usePrompt: string; demo: string; prompt: string }
  ...
  zh: {
    ...
    workspace: {
      ...
      onboarding: {
        ...
        style: { title: '确认你的工作风格', subtitle: '查看 Friday 从你的工作材料中提炼出的工作习惯。', summary: 'Friday 从你的工作材料里提炼出四类习惯。', points: ['表达：简洁直接；不确定会说清楚。', '判断：先确认事实、风险与负责人。', '下一步：更新后说清下一步。', '边界：承诺、敏感事项或信息不足，交给你。'], benefits: { summary: '清晰的工作风格能让 Friday 的回复更稳妥、更有用。', items: ['清晰回复', '贴合你的工作', '遵守边界'] }, extract: '蒸馏工作风格', skip: '跳过', extractingTitle: '正在蒸馏你的工作风格', extracting: ['正在梳理已连接的工作上下文…', '正在识别你的表达习惯…', '正在确认你对风险与负责人的判断方式…', '正在生成初版工作风格 Prompt…'], editHint: '确认前可直接修改这份工作风格。', promptLabel: '工作风格 Prompt', usePrompt: '使用这个 Prompt', demo: '仅为本地演示：此预览不会被持久保存或发送。', prompt: '表达方式\n使用简洁语言，直接说明不确定性。\n\n决策顺序\n先确认当前事实、风险、负责人和截止时间，再做出承诺。\n\n场景规则\n项目和会议更新后，给出下一步与负责人。\n\n必须交由本人\n承诺、敏感内容和信息不足时必须交由本人。' },
