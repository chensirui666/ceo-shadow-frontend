...
| 角色 | 值 | 使用规则 |
| --- | --- | --- |
| `status-*-foreground` | 对应基础功能色本身 | 标签文字、圆点、柱状图段、浮层项目符号和状态边线；同一状态必须同色 |
...
因此，成功、待处理、失败三个状态在首页任意位置均以 `#00B89C`、`#FF9F1C`、`#FF4D4F` 作为可见主色，而不是出现新的低饱和近似绿、黄、红。颜色只作辅助：图例、浮层和行内状态必须保留文字。
